...
# Interfaz principal
def main():
    # comprobar si hay respuesta de google OAuth (code en la URL)
    if 'code' in st.query_params:
        try:
            code = st.query_params['code']
            st.success(f"Código de autorización recibido: {code[:5]}...")
            
            auth = GoogleFitAuth()
            credentials = auth.get_credentials(code)
            
            # Limpiar parámetros y forzar recarga
    ...
    # si usuario ya ha autenticado, se muestra el dashboard
    if 'google_fit_creds' in st.session_state:
        try:
            ...
            if not fit_data:
                st.error("""
                    No se encontraron datos de fitness. Por favor, verifica que:
                    1. La aplicación tenga permisos para acceder a tus datos de fitness
                    2. Los datos de fitness estén disponibles en tu cuenta de Google Fit
                """)
                return
        ...
        # Mostrar sección de actividad física si está habilitada    
            ...
            del st.session_state['google_fit_creds']
            st.rerun()
    ...
    # --- BLOQUE DE AUTENTICACIÓN ---
        ...
        try:
            ...
            st.warning("""
                ...
                No almacenamos tus credenciales, solo usamos tokens temporales.
            """)

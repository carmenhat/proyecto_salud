...
class DataAnalyzer:
    ...
    def analyze_sleep(self, sleep_df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            required_columns = ['sleep_type', 'duration', 'date']
            ...
            # Convertir duration a numérico si no lo es
            if not pd.api.types.is_numeric_dtype(sleep_df['duration']):
                sleep_df['duration'] = pd.to_numeric(sleep_df['duration'], errors='coerce')
            
            # Eliminar filas con valores nulos
            sleep_df = sleep_df.dropna(subset=['sleep_type', 'duration'])
            
            # Filtrar solo los tipos de sueño relevantes (excluyendo despierto)
            ...
            unique_dates = pd.to_datetime(valid_sleep_df['date']).dt.date.nunique()
